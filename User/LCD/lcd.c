...
// 在 lcd.c 中添加一个同步版本的批量写入函数
void LCD_Color_Fill_Sync(uint16_t xsta, uint16_t ysta, uint16_t xend, uint16_t yend, uint16_t *color_p)
{
    ...
    LCD_Address_Set(xsta, ysta + OFFSET_Y, xend, yend + OFFSET_Y);
    HAL_SPI_Transmit(&hspi1, (uint8_t*)color_p, byte_count, HAL_MAX_DELAY);
}
